...
describe('cartReducer', () => {
	it('returns null for remove/update when cart is null', () => {
		...
		expect(
			cartReducer(null, { type: 'update', productId: 'x', quantity: 1 })
		).toBeNull()
	})

	it('creates a temporary cart on add when cart is null', () => {
		const result = cartReducer(null, {
			type: 'add',
			productId: 'test',
			quantity: 2
		})
	...
	it('removes an item', () => {
		const result = cartReducer(mockCart, {
			type: 'remove',
			productId: 'bottle_001'
		})
		expect(result?.items).toHaveLength(0)
...
describe('cartReducer rollback scenarios', () => {
	it('remove then rollback restores original cart', () => {
		const removed = cartReducer(mockCart, {
			type: 'remove',
			productId: 'bottle_001'
		})
	...
	it('update then rollback restores original quantity', () => {
		const updated = cartReducer(mockCart, {
			type: 'update',
			productId: 'bottle_001',
			quantity: 99
		})
